...
"""
function RmZeroVector(dataframe::DataFrame)
  #  获取dataframe的一维二维长度
  a, b = size(dataframe)
  #  创建一个与行数长度相同的0.0向量
  zerov = fill(0.0, a)
...
print(dataframe,selectdf)
"""
function AllNoZero(dataframe::DataFrame)
  #  获取dataframe的一维二维长度
  a, b = size(dataframe)
  #  创建一个与行数长度相同的0.0向量
  zerov = fill(0.0, a)
  ...
  #  从第一列到最后一列极限循环
  for i in 1:b
    #  将循环所在列中的元素展开，与等长0向量中的元素的对应位置一一比较
    #  只要有一个元素为0,那么在布尔值向量中将会出现1个1元素（true）
    #  那么它的多个元素和就不可能为0,必然大于或等于1
    #  把此时的列数和名称推送到delnamedf中
    if sum(dataframe[:, i] .== zerov) != 0
      push!(delnamedf, (i, namev[i]))
    end
  end
  #  用Not删除全部存在0元素的向量的列
